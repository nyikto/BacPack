 ...
 * BacPack2.js
 * Winter 2015
 * Expanding on BackPack for New Frontiers: Mars Environment
 *
 * Notes:
 * "if", "if gene", "resource" are used interchangably
 * "then", "then gene", "output", "need", "produce" are used interchangably
 *
 * @author Vivien Chen, Rachel Kwon, Sam Mincheva, Evan Segreto
 */
...
const petriDishW = 270;
const petriDishH = 270;
...
const distanceToEdge = (rootW / 80) + 60;
const midpointX = 954;
...
var dash1 = createDashboard(0, rootH - rootW/3, rootW/2, rootW/3);
root.addChild(dash1);
dash1.raiseToTop();
/*
var petri1 = createPetriDish(distanceToEdge, rootH /4 + 50, Math.PI / 2, 1);
var petri2 = createPetriDish((2 * rootW / 7) - (petriDishW / 2) + 50, rootH - distanceToEdge - petriDishH, 0, 1);
var petri3 = createPetriDish((5 * rootW / 7) - (petriDishW / 2) - 50, rootH - distanceToEdge - petriDishH, 0, 0);
var petri4 = createPetriDish(rootW - distanceToEdge - petriDishW, rootH /4 + 50, 3 * Math.PI / 2, 0);
...
function createPetriDish(x, y, rotation, flaskSide) {
 ...
 w.plasmidIndication = createPlasmidIndication(2 * w.width() / 3, 2 * w.height() / 3, w.width() / 6, w.height() - 60);
...
function createMarkerSensor(w) {
 ...
 markerSensor.setLocation(40, petriDishH - 25);
 markerSensor.setWidth(2 * petriDishW/3);
 markerSensor.setHeight(petriDishH/3);
...
function createPlasmidIndication(width, height, x, y) {
 ...
 if (w.strands.load(dnaImage)){
   w.strands.setWidth(w.width() * 2);
   w.strands.setHeight(w.height()/2);
   w.strands.setLocation(-w.width()/2, w.height()/4);
